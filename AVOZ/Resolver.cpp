...
void Manual(CUserCmd *pCmd) {
	if (Menu::Window.RageBotTab.ManualBox.GetState()) {
		static bool switchside = false;
		static float resttime;
		int SwitchSideKey = Menu::Window.RageBotTab.ManualKey.GetKey();
		if (SwitchSideKey >= 0 && GUI.GetKeyState(SwitchSideKey) && abs(resttime - Interfaces::Globals->curtime) > 0.5)
		{
			switchside = !switchside;
			resttime = Interfaces::Globals->curtime;
		}
		pCmd->viewangles.y += (switchside) ? -90 : 90;
	}
}
